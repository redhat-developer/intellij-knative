	...
	try {
		...
		if(params.UPLOAD_LOCATION) {
			stage('Upload') {
				...
				sh "rsync -Pzrlt --rsh=ssh --protocol=28 \"${filesToPush[0].path}\" ${UPLOAD_LOCATION}/snapshots/intellij-knative/"
			...
			stage("Publish to Marketplace") {
				...
				if (!isSnapshot) {
					stage("Promote the build to stable") {
						...
						sh "rsync -Pzrlt --rsh=ssh --protocol=28 \"${zip[0].path}\" ${UPLOAD_LOCATION}/stable/intellij-knative/"
