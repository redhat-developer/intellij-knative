...
node('rhel7'){
	...
	try {
		stage('Checkout repo') {
			deleteDir()
			git url: 'https://github.com/redhat-developer/intellij-knative',
				branch: "${sha1}"
		}
		...
		if(params.UPLOAD_LOCATION) {
			stage('Upload') {
				...
				sh "sftp -C ${UPLOAD_LOCATION}/snapshots/intellij-knative/ <<< \$'put -p \"${filesToPush[0].path}\"'"
		...
		if(publishToMarketPlace.equals('true')){
			timeout(time:5, unit:'DAYS') {
				input message:'Approve deployment?', submitter: 'jmaury,lstocchi,adietish'
			}
			...
			stage("Publish to Marketplace") {
				...
				if (!isSnapshot) {
					stage("Promote the build to stable") {
						...
						sh "sftp -C ${UPLOAD_LOCATION}/snapshots/intellij-knative/ <<< \$'put -p \"${zip[0].path}\"'"
